...
Usage (CLI):
  python audio_visualizer.py --audio path/to/file.mp3 --style bars --palette rainbow \
      ...
      --progress-bar --progress-pos bottom --progress-height 10 \
      --clock digital --clock-mode system --clock-style dark --clock-pos tr --clock-scale 1.0
...
def _overlay_progress_bar(img: np.ndarray, progress: float, position: str = 'bottom', height: int = 10,
    ...
    """Draw a simple filled progress bar. progress in [0,1]."""
    h, w = img.shape[:2]
...
class RenderSettings:
    ...
    clock_mode: str = 'system'     # 'system' or 'elapsed'
...
def render_video(settings: RenderSettings) -> str:
    ...
    for idx in range(total_frames):
        ...
        if settings.clock:
            if settings.clock_mode == 'elapsed':
                ...
                if settings.clock == 'digital':
                    frame = _overlay_digital_clock(frame, _format_hms(elapsed), settings.clock_pos, settings.clock_style, settings.clock_scale)
                else:
                    # synthesize a datetime from elapsed (00:00 + elapsed)
        ...
        if settings.progress_bar:
            ...
            frame = _overlay_progress_bar(frame, progress, settings.progress_pos, settings.progress_height, progress_color)
...
if TK_AVAILABLE:
    ...
    class AudioVisualizerApp:
        def __init__(self, root):
            ...
            # clock
            self.clock_enable_var = tk.BooleanVar(value=False)
            ...
            self.clock_mode_var = combo(overlays, "Режим:", ["system","elapsed"], "system", 3)
...
def parse_args(argv=None):
    ...
    p.add_argument('--clock', default=None, choices=['digital','analog'])
    p.add_argument('--clock-mode', default='system', choices=['system','elapsed'])
...
class TestFrames(unittest.TestCase):
    ...
    def test_digital_clock_overlay(self):
        img = np.zeros((self.h, self.w, 3), dtype=np.uint8)
        out = _overlay_digital_clock(img.copy(), '12:34', 'tr', 'dark', 1.0)
