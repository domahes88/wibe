...
- **NEW**: Batch processing — render **multiple audio files** and (optionally) **merge into one video** with **chapters** (bookmarks) named after filenames.
...
Usage (CLI):
  ...
  # Multiple files → one video with chapters (bookmarks per file name)
  python audio_visualizer.py --audio song1.mp3 song2.mp3 song3.wav \
      --merge --chapters --out-dir out/

  # Multiple files → render separately (no merge)
  python audio_visualizer.py --audio folder/*.mp3 --no-merge
...
def _ffmetadata_for_chapters(titles: List[str], durations_sec: List[float]) -> str:
    """Create FFmetadata text with chapters named by titles. TIMEBASE=1/1000, START/END in ms."""
...
def render_batch(audio_paths: List[str], base_settings: RenderSettings, merge: bool = True, add_chapters: bool = True) -> List[str] | str:
    """Render multiple inputs.
    - If merge=True → returns path to final merged video with chapters (if add_chapters=True).
    - If merge=False → returns list of individual output paths.
    """
    if not audio_paths:
    ...
    # Concat all segments into one — require ffmpeg
...
if TK_AVAILABLE:
    ...
    class AudioVisualizerApp:
        def __init__(self, root):
            ...
            self.files: List[str] = []  # multiple files
        ...
        def start_render(self):
            ...
            base = RenderSettings(
                audio_path=self.files[0],  # will be replaced per file in batch
